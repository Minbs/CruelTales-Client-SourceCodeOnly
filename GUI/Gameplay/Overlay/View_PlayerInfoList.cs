	...
	public class View_PlayerInfoList : ViewBaseWithContext
	{
		...
		private void sortPlayerInfoTable()
		{
			...
			foreach (var item in sortedTable)
			{
				...
				//_contextItemByID[item.Key] = item.Value;
				//_contextItemByID[item.Key].Context.OnChanged(item.Value.Context);
			}

			/*
			_contextItemByID = sortedTable.ToDictionary(x => x.Key, x => x.Value);
			_contextItemByID[GlobalService.BackendManager.UserId].GameObject.transform.SetSiblingIndex(0); // 자기 자신은 0

			int count = 1;

			if (PlayerListTransform.childCount < 2)
				return;

			foreach (var item in _contextItemByID)
			{
				item.Value.GameObject.transform.SetSiblingIndex(count); // Dictionary 순서대로 트랜스폼 정렬
				count++;
			}
			*/
		}
