...
namespace CTC.GUI.Gameplay.Common.PlayerNameTag
{
	public class Item_UserNameTag : MonoBehaviour
	{
		...
		IEnumerator SetIconImagePosition()
		{
			...
			/*
			Vector3 nextPosition = _rectTransform.transform.position;
			nextPosition.x = nextPosition.x - TotalWidth * 0.5f + PlayerIconTransform.rect.x * 0.5f;
			PlayerIconTransform.transform.position = nextPosition;
			nextPosition.x = nextPosition.x + TextPadding.x - PlayerNameText.rectTransform.rect.x / 2;
			PlayerNameText.rectTransform.transform.position = nextPosition;
			*/
		}
	}
}
